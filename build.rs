...
/// `data/language.toml`, replacing the old `hover.toml` and `completions.toml`
/// pipelines.
///
/// `hover.rs` defines:
/// ```rust
...
/// ```
/// Registers `cargo:rerun-if-changed` for `data/language.toml` and
/// `data/hover_docs/`. Delegates all parsing and code generation to the
/// `language` submodule.
fn generate_from_language_toml(out_dir: &str) {
    println!("cargo:rerun-if-changed=data/language.toml");
    println!("cargo:rerun-if-changed=data/hover_docs/");
    language::generate(out_dir);
}
...
fn main() {
    ...
    generate_from_language_toml(&out_dir);
}
